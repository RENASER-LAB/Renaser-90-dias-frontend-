...
2. **Vista previa en desarrollo, y apagado en producción hasta que se encienda.** En builds `__DEV__` la tarjeta aparece siempre (marcada "VISTA PREVIA"), para poder probarlo sin esperar una semana de programa. En el build de Vercel/tiendas **no aparece nunca** salvo que la variable `EXPO_PUBLIC_MAPA_DIA7=on` esté definida al compilar — y aun así, solo desde el Día 7. Motivo: la rama `auditoria` se mergea con usuarios en curso y el mapa todavía no tiene backend (§4); mergear no debe ser lo mismo que lanzar. Encenderlo es una decisión: poner la variable en Vercel y redesplegar.
